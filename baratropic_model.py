...
def a(m,b):
    return 8.0*sqrt(2)*(m^2)*(b**2 + m**2 -1)/(math.pi*(-1+ 4.0*m**2)*(b**2 + m**2))
...
def gammaprime(m,b,g):
    return g*4*sqrt(2)*b*m/((4*(m**2)-1)*math.pi);

              
def gamma(m,b,g):
    return g*4*sqrt(2)*b*(m**3)/((4*(m**2)-1)*pi*(b**2 + m**2));


def d(m,b):
    return 64*sqrt(2)*(b**2 - m**2 +1)/(15*pi*(b**2 + m**2));
    
    

x_init=[] #the initial conditions of the model
dt=2e-4 #the time step used by the euler maruyama scheme
Nsample=1 #the temporal sampling rate of the data in units of dt
Nmax=10 #how many samples you want in the final data
sigma=1.0 #the stochastic forcing coefficient
...
e=16.0*sqrt(2.0)/(5.0*math.pi) #a nonlinear advection coefficient
...
myrng=RandomState(int(time.time()))

    
for N in range(1,Nmax):
    
    x=state[N-1]
    dW=myrng.normal(0.0,sqrt(dt),[Nsample,len(xdot)])#initialise random numbers for next set of timesteps
    
    for t in range(0,Nsample):
        #linear part done with matrix method
        xdot=Lin_Op.dot(x)+xf
...
if SAVE:
    tx=[time,state]
    filepath='baratropic_euler_stoch_sigma%.2f.csv' % sigma
    df=pd.DataFrame(tx)
    df.to_csv(filepath)
