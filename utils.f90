...
module utils
	...
	implicit none
	private
	public time_seed, white_noise, red_noise, randomise_init_con
	...
	contains
		...
		!makes random initial array, each variable drawn from normal distribution
		subroutine randomise_init_con(init_con_arr)
			real(dp) :: init_con_arr(dims)
			real(dp) :: rand1(dims), rand2(dims),u(dims),v(dims)
